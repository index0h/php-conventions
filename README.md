...
> В случае использования "тонких" ActiveRecord моделей, помимо данных объект модели содержит подключение к базе данных.
> Код, использующий такую модель получает неявную зависимость от базы данных. Что бы покрыть unit тестами такой код,
> с большой вероятностью потребуется менять поведение драйвера подключения к базе данных, что приведет к крайне высокой 
> сложности поддержки.

```php
// Пример создан на основании руководств Yii2 и Laravel
...
> Подход к работе с моделями, описанный в примере МОЖЕТ привести к проблемам безопасности, так как фактическая работа
> с базой данных МОЖЕТ не явно выполняться в произвольном месте, например в шаблонах.
>
> "Толстые" ActiveRecord модели включают в себя проблемы "тонких", а так же МОГУТ привести к антишаблону God object,
> что существенно усложнит разработку и поддержку. К частым проблемам "толстых" ActiveRecord моделей так же стоит
> отнести сложности разделения бизнес логики, связывающей вместе несколько таких моделей.
