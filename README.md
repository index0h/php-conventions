...
> В случае использования ActiveRecord, мы связываем вместе сущности, которые представляют наш домен, и инфраструктуру.
> В данном случае логику сохранения данных в базу. Это противоречит Single Responsibility Principle из SOLID, сильно усложняет изолированное тестирование моделей(из-за того что нужно мокать подключение к БД), нарушает Interface Segregation Principle, так как интерфейс домена смешивается с интерфейсом инфраструктуры, что влечёт за собой утерю контроля над тем, у каких модулей есть доступ к инфраструктуре хранилища, большее количество зависимостей от модели(больше классов нужно просмотреть при изменениях в модели), и потенциальные проблемы с безопасностью, т.к. любой модуль использующий любую сущность может выгрузить любые данные из базы.
